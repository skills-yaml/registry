...
metadata:
  ...
  skm-source-integrity: "sha256:9fb571a9312cea82aec70aafa670272005f832cd9dc8040419754eb3c07faaf0"
...
```sh
task peers -- join --agent worker-api --task api-change --scope backend/api
task peers -- join --agent worker-ui --task ui-change --scope frontend/app
task peers -- status
```
...
actor operations. No coordinator allocates or approves the claim. Repeat
`--scope` for multiple paths and `--depends-on` for prerequisite task IDs;
prerequisites must be integrated before a dependent task joins.
