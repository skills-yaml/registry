...
- `v6.0.0` is an unreleased candidate that adds linked-worktree isolation and
  ...
  version impact and shared SemVer release reservations. It is available for
  repository validation but is not yet a published default.
...
The version 6.0 candidate preserves that lifecycle and requires each concurrent
mutating agent to use a dedicated linked worktree plus a repository-visible WIP
record. Worktrees are detached by default; task or delivery branches require a
direct user request for the named task.

When the repository includes the
[`adopt-workspace-structure` skill](../../SKILL.md),
